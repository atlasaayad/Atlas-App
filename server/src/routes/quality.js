...
qualityRouter.put('/models/:id', (req, res) => {
  const { id } = req.params
  ...
  db.prepare(
    `INSERT INTO quality (model_id, percentage, reprises, updated_at) VALUES (?, ?, ?, ?)
  ...
  ).run(id, percentage, reprises, now)
  logAudit({ deptKey: 'quality', modelId: id, action: 'update_quality', details: { percentage, reprises } })
  res.json({ ok: true })
})
