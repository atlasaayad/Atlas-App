...
async function recompute(modelId) {
  const gamme = await all('SELECT tps FROM gamme_lines WHERE model_id = $1', [modelId])
  const effectifRows = await all('SELECT required FROM effectif_requis WHERE model_id = $1', [modelId])
...
methodeRouter.post('/models', async (req, res) => {
  ...
  for (const spec of SPECIALTIES) {
    await run('INSERT INTO effectif_requis (model_id, specialty, required) VALUES ($1, $2, 0)', [id, spec])
  }
  for (let i = 0; i < 9; i++) {
...
methodeRouter.put('/models/:id/gamme', async (req, res) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    const l = lines[i]
    await run('INSERT INTO gamme_lines (id, model_id, seq_no, operation, machine, tps) VALUES ($1, $2, $3, $4, $5, $6)', [
      `gml_${nanoid(10)}`,
      req.params.id,
      i + 1,
      l.operation || '',
...
methodeRouter.put('/models/:id/effectif', async (req, res) => {
  ...
  for (const spec of SPECIALTIES) {
    ...
    await run(
      `INSERT INTO effectif_requis (model_id, specialty, required) VALUES ($1, $2, $3)
       ON CONFLICT (model_id, specialty) DO UPDATE SET required = excluded.required`,
      [req.params.id, spec, required]
    )
  }
