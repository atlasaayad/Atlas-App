...
export default function Lessons({ lessons }) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-lg font-semibold text-[#D4AF37]">{t('lessonsTitle')}</h2>
        <p className="text-sm text-slate-400">{t('lessonsDesc')}</p>
        <p className="mt-1 text-xs text-slate-500">
          <span className="font-semibold text-[#D4AF37]">{t('referenceSet')}:</span> {t('referenceNote')}
        </p>
      </div>
      ...
      {lessons.length === 0 ? (
        ...
        <div className="space-y-3">
          {lessons.map((lesson) => (
            <div key={lesson.id} className="rounded-xl border border-[#D4AF37]/15 bg-navy-900/50 p-4">
              <div className="flex items-center justify-between gap-2">
                ...
                <span
                  className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${
                    lesson.kind === 'reference' ? 'bg-[#D4AF37]/15 text-[#D4AF37]' : 'bg-status-bad/15 text-status-bad'
                  }`}
                >
